			...
			if (mysqli_num_rows($result)) {
				while ($row = mysqli_fetch_assoc($result)) {
					if ($editid == md5($row['invoice_no'])) {
						...
						$date_time = date('F d, Y H:i', strtotime($created_at));
						$isExist++;
					}
				}
			}
				...
				<div id="demand_layout">
					...
					<div id="page_heading" class="row">
						...
						<div class="d-flex align-items-center heading_number">
							(
								<span> #</span>	
								<span> <?php echo mraCreateSixDigitNumber($invoice_no); ?> </span>
							)
						</div>
					</div>
					<div id="margin_div"></div>

					<form id="editinvoice_form">
						...
						<div class="row mt-1">
							<div class="col-md-3 mt-1">
								...
								<input id="date" name="date" class="form-control form-control-sm date" value="<?php echo $date_time; ?>" readonly>
							</div>
							<div class="col-md-3 mt-1">
								<span class="font-weight-light">
									<strong> Client:<i class="text-danger">*</i> </strong>
								</span>
								<select id="client_id" name="client_id" class="form-control form-control-sm client_id" readonly>
					  	...
					  	<div class="mt-3" style="box-shadow: 0px 0px 8px 0px #bebebe; padding: 8px 15px; background: #FBFCFC; border-radius: 5px; margin-top:12px !important; padding-bottom: 18px;">
					  		<div class="d-flex justify-content-between">
					  			<div> <h4 id="subform_demand_products"> Items List </h4> </div>
					  			<!-- <div>
					  				<a class="btn btn-sm btn-outline-success" href="javascript:void(0)" data-toggle="modal" data-target="#LoadItemsInModal"> Add </a>
					  			</div> -->
					  		</div>
					  		<?php //include_once("LoadItemsInModal.php"); ?>
							
							<div style="overflow: auto;">
							  	<table id="QuotationListTable" class="table-sm table-hover table-striped table_form_style" style="min-width: 1000px;">
							  		<thead class="card-header">
							  			<tr>
							  				<th width="40px" class="text-success">
							  					<i class="fa fa-check"></i>
							  				</th>
							  				<th width="40px"> # </th>
							  				<th width="220px"> Item Description </th>
							  				<th width="180px"> Specification </th>
							  		...
							  		<tbody>
							  			...
							  			<?php
							  				$count = 1;
							  				$query4 = "Select * FROM invoice_items WHERE invoice_no = '$invoice_no' && status = '1'";
							  			...
							  			?>
							  				<tr class="<?php echo $readonly_class; ?>">
							  					<td>
							  						<input type="checkbox" name="row_id[]" class="row_id" checked>
							  						<input type="hidden" name="is_changed[]" class="is_changed" value="1">
							  						<input type="hidden" name="item_row_id[]" class="item_row_id" value="<?php echo $row_id; ?>">
							  						<input type="hidden" name="product_id[]" class="product_id" value="<?php echo $product_id; ?>">
							  					</td>
							  					<td class="numbers_counter"> <?php echo $count++; ?> </td>
							  					<td> <input type="text" id="description" name="description[]"  class="form-control form-control-sm description" value="<?php echo $description; ?>" readonly>
												</td>
							  					<td> <input type="text" id="specification" name="specification[]" class="form-control form-control-sm specification" value="<?php  echo $specification; ?>" readonly> </td>
							  					<td> <input type="text" id="qty" name="qty[]" class="form-control form-control-sm qty" placeholder="1" oninput="<?php echo $onlyNumeric; ?>" value="<?php echo $qty; ?>" maxlength="12"> </td>
							  					<td> <input type="hidden" id="uom" name="uom[]" class="uom" value="<?php echo $uom; ?>">
													<select id="unit_of_measurement" class="form-control form-control-sm unit_of_measurement" name="unit_of_measurement">
														<option value="" disabled selected hidden> Choose </option>
														<?php
												...
												<td> <input type="text" id="unit_price" name="unit_price[]" class="form-control form-control-sm unit_price" placeholder="100.0" oninput="<?php echo $onlyNumeric; ?>" value="<?php echo $unit_price; ?>" maxlength="12"> </td>
												<td> <input type="text" id="total" name="total[]" class="form-control form-control-sm total" value="<?php echo $line_total; ?>" readonly> </td>
												<td> <input type="text" id="additional_note" name="additional_note[]" class="form-control form-control-sm additional_note" value="<?php echo $additional_note; ?>" maxlength="50"> </td>
							  				</tr>
							  			<?php
							  					}
							  				}
						...
						<div class="row mt-1" style="font-size: 15px;">
							...
							<div class="col-md-6 mt-1">
								...
								<input type="" id="po_no" name="po_no" class="form-control form-control-sm po_no" placeholder="20240209-001010-01" oninput="<?php echo $forInvoiceNo; ?>" maxlength="18" value="<?php if(isset($po_no)) { echo $po_no; } ?>">
							</div>
						</div>
					  	...
					  	<div class="col-md-12 p-0 mt-1">
					    	...
					    	<textarea rows="3" name="comment" form="editinvoice_form" class="form-control form-control-sm comment" placeholder="Aa" maxlength="299"><?php if ( isset($comment)) { echo $comment; }?></textarea>
					  	</div>
					  	
					   <input type="hidden" name="action" value="updateInvoice" />
					  	<div class="form-group text-center mt-3">
					    	<input type="submit" id="form_submit_button" name="update_invoice" value="Generate" class="update_invoice">
					  	</div>
					</form>
		</div>
		</div>
...
<script type="text/javascript" src="js/editinvoice.js?clear_cache=<?php echo time();?>"></script>
