<?php
	...
	if($_SESSION['user_email'] == true) {
		...
		if ($_SESSION['user_role'] == "admin") {
			...
			// $dateTime = date('F d, Y H:i', strtotime('+3 Hours'));
			$dateTime = date('F d, Y H:i', strtotime('+3 Hours'));


			$query = "Select * FROM draft WHERE username = '$username' && type = 'purchase'";
			$result = mysqli_query($connection, $query);
			$isDraft = mysqli_num_rows($result);
			if (mysqli_num_rows($result)) {
				while ($row = mysqli_fetch_assoc($result)) {
					$username = $row['username'];
					$dbvendor_id = $row['vendor_id'];
				...
				$query = "Select * FROM draft_items WHERE username = '$username' && type = 'purchase'";
				$result = mysqli_query($connection, $query);
				$isDraft = mysqli_num_rows($result);
			}
?>
			...
			<div id="inner_layout">
				<div id="demand_layout">
					...
					<div id="page_heading" class="row d-flex justify-content-between">
						...
						<div style="font-size:14px; padding-top:16px;" id="empty_draft">
							<?php if ($isDraft) { ?>
								<span> <i class="fa fa-trash-o" style="font-size:15px;"></i> Draft </span>
							<?php } ?>
						</div>
					</div>
					...
					<form id="purchase_form">
						...
						<div class="row mt-1">
							...
							<div class="col-md-3 mt-1">
								...
								<select id="vendor_id" name="vendor_id" class="form-control form-control-sm vendor_id">
						...
						<input type="hidden" id="generated_by" name="generated_by" value="<?php echo $username; ?>">
					  	...
					  	<div class="mt-3" style="box-shadow: 0px 0px 8px 0px #bebebe; padding: 8px 15px; background: #FBFCFC; border-radius: 5px; margin-top:12px !important; padding-bottom: 18px;">
					  		<div class="d-flex justify-content-between">
					  			<div> <h4 id="subform_demand_products"> Make a purchase list </h4> </div>
					  			<div>
									  <a class="btn btn-sm add_row" href="javascript:void(0)" data-toggle="modal" data-target="#LoadItemsInModal"> Add </a>
					  			</div>
					  		</div>
							 
							<?php include_once("LoadItemsInModalForNewPurchase.php"); ?>
							<div style="overflow: auto;">
							  	<table id="PurchaseListTable" class="table-sm table-hover table-striped table_form_style" style="min-width: 1000px;">
							  		...
							  		<tbody id="purchase_items">
							  			...
							  			<?php
							  				$count = 1;
							  				$query4 = "Select * FROM draft_items WHERE username = '$username' && product_id <> '' && type = 'purchase'";
							  			...
							  			?>
							  				<tr class="<?php echo $readonly_class; ?>">
												...
												<td> <input type="text" id="unit_price" name="unit_price[]" class="form-control form-control-sm unit_price" placeholder="100.0" oninput="<?php echo $onlyNumeric; ?>" value="<?php if(isset($unit_price)) { echo $unit_price; } ?>" maxlength="18" <?php echo $readonly; ?>> </td>
						...
						<div class="row mt-1" style="font-size: 15px;">
							...
							<div class="col-md-6 mt-1">
								...
								<input type="" id="reference_no" name="reference_no" class="form-control form-control-sm reference_no" placeholder="Document #123456 " maxlength="18">
							</div>
						</div>
					  	...
					  	<div class="col-md-12 p-0 mt-1">
					    	...
					    	<textarea rows="3" name="comment" form="purchase_form" class="form-control form-control-sm comment" placeholder="Aa" maxlength="299"><?php 
			      					if ( isset($comment)) { echo $comment; }
			      				?></textarea>
					  	</div>
					  	
					   <input type="hidden" name="currency" class="currency" value="<?php if(isset($currency)) { echo $currency; } ?>" />
					   <input type="hidden" name="action" value="savePurchase" />
					  	<div class="form-group text-center mt-3">
					    	<input type="submit" id="form_submit_button" name="generate_purchase" value="Generate" class="generate_purchase">
					  	</div>
					  	<input type="hidden" class="cntr" value="<?php echo $count-1; ?>">
					</form>
		</div>
		</div>
		</div>
...
<script type="text/javascript" src="js/createpurchase.js?clear_cache=<?php echo time();?>"></script>
