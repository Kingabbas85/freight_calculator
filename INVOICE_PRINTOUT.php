<?php
	...
	$invoice_no = $client_id = $payment_mode = $credit_terms = $currency_unit = $date = '';
	$query = "Select * FROM invoices WHERE invoice_no = '$id'";
	...
	if (mysqli_num_rows($result3)) {
		while ($row3 = mysqli_fetch_assoc($result3)) {
			...
			$client_name = ucfirst($row3['client_name']);
			$client_contact_name = ucfirst($row3['contact_name']);
			$client_address = $row3['address'];
			$client_contact_no = $row3['contact_no'];
			$client_ntn = $row3['ntn_number'];
		}
	}
	if ($client_name == "") {
		$client_name = "-";
	...
	class myPDF extends FPDF {
		...
		function footer() {
    		...
    		$this->Cell(200,4,'If you have any questions concerning this invoice, Contact  Email, muhammadsameer.mra@gmail.com',"",1,'L', true);
    		$this->Cell(200,1,'',"",1,'L', true);
	...
	$pdf->SetFont('helvetica', 'B', 8.5);
	$pdf->Cell(32,6.5,'Invoice #','LRB',0,'L');
	$pdf->SetFont('helvetica', '', 8);
	$pdf->Cell(32,6.5,mraCreateSixDigitNumber($invoice_no),'RB',1,'L');
	...
	$pdf->Cell(200,11.5,'Invoice','1',1,'C', false);
	...
	$counter = 1;
	$query3 = "Select * FROM invoice_items WHERE invoice_no = '$invoice_no' && status = '1'";
	$result3 = mysqli_query($connection, $query3);
	if (mysqli_num_rows($result3)) {
		while ($row3 = mysqli_fetch_assoc($result3)) {
			...
			if (mysqli_num_rows($result4)) {
				while ($row4 = mysqli_fetch_assoc($result4)) {
					$description = ucwords($row4['description']);
					$specification = ucfirst($row4['specification']);
				}
	...
	$invoice_no = "NV_".mraCreateSixDigitNumber($invoice_no).".pdf";
	$pdf->Output("assets/invoices/".$invoice_no,"F");

	$filePath = "assets/invoices/".$invoice_no;
