			...
			<div id="inner_layout">
				<div id="quotation_layout">
					...
					<div id="page_heading" class="row d-flex justify-content-between">
						<div> Quotations </div>
						<div id="add_new">
							<a href="NewQuotation"> <i class="fa fa-plus"></i>&nbsp; New Quotation </a>
						</div>
					</div>
					...
					<div id="table_layout_updated">
						<table id="QuotationTable" class="table table-sm table-striped table-hover w-100 table_style"
							...
							<?php
								$count = 1;
								$query = "Select * FROM quotations WHERE status = '1' ORDER By quotation_no DESC";
								...
								if (mysqli_num_rows($result) > 0) {
									while ($row = mysqli_fetch_assoc($result)) {
										...
										$file_name = "assets/quotations/QT_".mraCreateSixDigitNumber($quotation_no).".pdf";
										...
										echo '<tr>';
											...
											echo '<td>';
												echo '<div class="d-flex justify-content-between">';
													echo '<div>'.mraCreateSixDigitNumber($quotation_no).'</div>';
											...
											echo '<td class="text-right">';
												echo '<input type="hidden" class="id" value="'.mraCreateSixDigitNumber($quotation_no).'">';
												echo '<div class="btn-group">
													...
													echo '<a class="dropdown-item" href="'.$file_name.'" download>
													...
													if (!$is_closed) {
													  	...
													  	echo '<a class="dropdown-item generate_invoice">
														   	<i class="fa fa-plus text-secondary"></i>&nbsp; Generate Invoice
														   </a>';
												  	}
												  	if ($is_closed) {
...
<script type="text/javascript" src="js/quotations.js?clear_cache=<?php echo time(); ?>"></script>
<?php
		} else {
