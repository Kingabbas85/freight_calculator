<?php
	...
	if($_SESSION['user_email'] == true) {
		...
		if ($_SESSION['user_role'] == "admin") {
			...
			$query = "Select * FROM draft WHERE username = '$username' && type = 'quotation'";
			...
			if (mysqli_num_rows($result)) {
				...
				$query = "Select * FROM draft_items WHERE username = '$username' && type = 'quotation'";
				$result = mysqli_query($connection, $query);
				$isDraft = mysqli_num_rows($result);
			}
?>
					...
					<form id="quotation_form">
						...
						<input type="hidden" id="generated_by" name="generated_by" value="<?php echo $username; ?>">
					  	...
					  	<div class="mt-3" style="box-shadow: 0px 0px 8px 0px #bebebe; padding: 8px 15px; background: #FBFCFC; border-radius: 5px; margin-top:12px !important; padding-bottom: 18px;">
							 ...
							 <?php include_once("LoadItemsInModalForNewQuotation.php"); ?>
							<div style="overflow: auto;">
							  	<table id="QuotationListTable" class="table-sm table-hover table-striped table_form_style" style="min-width: 1000px;">
							  		...
							  		<tbody id="quotation_items">
							  			...
							  			<?php
							  				$count = 1;
							  				$query4 = "Select * FROM draft_items WHERE username = '$username' && product_id <> '' && type = 'quotation'";
						...
						<div class="row mt-1" style="font-size: 15px;">
					  	...
					  	<div class="col-md-12 p-0 mt-1">
					    	...
					    	<textarea rows="3" name="comment" form="quotation_form" class="form-control form-control-sm comment" placeholder="Aa" maxlength="299"><?php 
			      					if ( isset($comment)) { echo $comment; }
			      				?></textarea>
					  	</div>
					  	
					   <input type="hidden" name="currency" class="currency" value="<?php if(isset($currency)) { echo $currency; } ?>" />
					   <input type="hidden" name="action" value="saveQuote" />
					  	<div class="form-group text-center mt-3">
					    	<input type="submit" id="form_submit_button" name="generate_quote" value="Generate" class="generate_quote">
					  	</div>
...
<script type="text/javascript" src="js/createquotation.js?clear_cache=<?php echo time();?>"></script>
