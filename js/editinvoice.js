$(document).ready(function () {
	...
	$("#editinvoice_form").delegate(".row_id", "click", function(e) {
	...
	$("#editinvoice_form").delegate(".qty", "focusin", function() {
	...
	$("#editinvoice_form").delegate(".qty", "focusout", function() {
	...
	$("#editinvoice_form").delegate(".qty", "keyup", function() {
	...
	$("#editinvoice_form").delegate(".unit_of_measurement", "change", function() {
		var uom = $(this).val();
		var tr = $(this).parent().parent();
		tr.find(".uom").val(uom);
	...
	$("#editinvoice_form").delegate(".unit_price", "focusin", function() {
		
		if ($(this).val() == 0) {
			$(this).val("");
		}
	});
	$("#editinvoice_form").delegate(".unit_price", "focusout", function() {
	...
	$("#editinvoice_form").delegate(".unit_price", "keyup", function() {
	...
	$("#editinvoice_form").on("focusin",".tax", function(e) {
		var tax = $(this).val();
		if (tax == 0) {
			$(this).val("");
		}
	});
	$("#editinvoice_form").on("focusout",".tax", function(e) {
		var tax = $(this).val();
		if (tax == "." || tax == "") {
			tax = 0;
		}
	...
	$("#editinvoice_form").on("keyup",".tax", function(e) {
		calculate();
	});
	$("#editinvoice_form").on("focusin",".discount", function(e) {
		var discount = $(this).val();
		if (discount == 0) {
			$(this).val("");
		}
	});
	$("#editinvoice_form").on("focusout",".discount", function(e) {
		var discount = $(this).val();
	...
	$("#editinvoice_form").on("keyup",".discount", function(e) {
		calculate();
	});
	$("#editinvoice_form").on("focusin",".delivery_charges", function(e) {
		var delivery_charges = $(this).val();
		if (delivery_charges == 0) {
			$(this).val("");
		}
	});
	$("#editinvoice_form").on("focusout",".delivery_charges", function(e) {
		var delivery_charges = $(this).val();
	...
	$("#editinvoice_form").on("keyup",".delivery_charges", function(e) {
		calculate();
	});
	...
	$("#editinvoice_form").on("keyup", ".comment", function(e) {
	...
	// Generate invoice
	$("#editinvoice_form").on("click", ".update_invoice", function(e) {
		e.preventDefault();
		// $(".overlay").show();

		var isRequired = 0;
		$("#editinvoice_form .qty").map(function(){
		...
		$("#editinvoice_form .unit_of_measurement").map(function(){
			if ($(this).val() == null) {
				$(this).css("border","1px solid red");
				$(this).css("box-shadow","0px 0px 5px 0px #F1948A");
				isRequired++;
		...
		$("#editinvoice_form .unit_price").map(function(){
			if ($(this).val() == "" || $(this).val() == 0) {
				$(this).css("border","1px solid red");
				$(this).css("box-shadow","0px 0px 5px 0px #F1948A");
				isRequired++;
		...
		if (isRequired) {
			...
			$.ajax({
				...
				method : "POST",
				data : $("#editinvoice_form").serialize(),
				// data : {
				// 	saveDemand:1,
				// 	client:client,
				// },	
