<?php
	...
	if($_SESSION['user_email'] == true) {
		...
		if ($_SESSION['user_role'] == "admin") {
			...
			$query2 = "Delete FROM purchase_items WHERE purchase_no = '$purchase_no' && status = 0";
				...
				<div id="demand_layout">
					...
					<div id="page_heading" class="row">
						...
						<div class="d-flex align-items-center heading_number">
							(
								<span> #</span>	
								<span> <?php echo mraCreateSixDigitNumber($purchase_no); ?> </span>
							)
						</div>
					</div>
					...
					<form id="editpurchase_form">
						...
						<input type="hidden" name="purchase_no" class="purchase_no" value="<?php echo $purchase_no; ?>">
					  	...
					  	<div class="mt-3" style="box-shadow: 0px 0px 8px 0px #bebebe; padding: 8px 15px; background: #FBFCFC; border-radius: 5px; margin-top:12px !important; padding-bottom: 18px;">
					  		...
					  		<?php include_once("LoadItemsInModalForEditPurchase.php"); ?>
							...
							<div style="overflow: auto;">
							  	<table id="QuotationListTable" class="table-sm table-hover table-striped table_form_style" style="min-width: 1000px;">
							  		...
							  		<tbody id="editpurchase_items">
							  			<tr></tr>
							  			<?php
							  				$count = 1;
							  				$query4 = "Select * FROM purchase_items WHERE purchase_no = '$purchase_no' && status = '1'";
						...
						<div class="row mt-1" style="font-size: 15px;">
							...
							<div class="col-md-6 mt-1">
								...
								<input type="" id="reference_no" name="reference_no" class="form-control form-control-sm reference_no" placeholder="Document #123456 " maxlength="18" value="<?php if(isset($reference_no)) { echo $reference_no; } ?>">
							</div>
						</div>
					  	...
					  	<div class="col-md-12 p-0 mt-1">
					    	...
					    	<textarea rows="3" name="comment" form="editpurchase_form" class="form-control form-control-sm comment" placeholder="Aa" maxlength="299"><?php if ( isset($comment)) { echo $comment; }?></textarea>
					  	</div>
					  	
					   <input type="hidden" name="currency" value="<?php echo $currency; ?>" />
					   <input type="hidden" name="edit_id" value="<?php echo $edit_id; ?>" />
					   <input type="hidden" name="deleted_ids" class="deleted_ids" />
					   <input type="hidden" name="action" value="updatePurchase" />
					  	<input type="hidden" class="counter" value="<?php echo $count-1; ?>">
					  	<div class="form-group text-center mt-3">
					    	<input type="submit" id="form_submit_button" name="update_purchase" value="Update" class="update_purchase">
					  	</div>
					</form>
		</div>
		</div>
...
<script type="text/javascript" src="js/editpurchase.js?clear_cache=<?php echo time();?>"></script>
